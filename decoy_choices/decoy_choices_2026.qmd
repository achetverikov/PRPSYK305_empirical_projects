...
```{r descr_stats}
data_descr <- data_out[,.(.N, mean_RT = mean(rt)), keyby = .(participant, decoy_present = decoy_on!='no_decoy', chosen_item)][,.(chosen_item, decoy_present, prop_choices = 100*N/sum(N), mean_RT), by = participant]
```
...
plot_pointrange(data_out, aes(x = condition, y = correct, color = decoy_on), wid = 'participant', design = 'w', within_subj =  T)+labs(y = 'Proportion of correct responses', color = 'Decoy is similar to...', x = 'Condition')+scale_color_manual(values = default_colors)
```
...
plot_pointrange(data_out, aes(x = condition, y = rt, color = decoy_on), wid = 'participant', design = 'w', within_subj = T)+labs(y = 'Response time, s', color = 'Decoy is similar to...', x = 'Condition')+scale_color_manual(values = default_colors)
```
