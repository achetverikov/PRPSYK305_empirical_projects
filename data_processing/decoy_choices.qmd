...
data <- data[, lapply(.SD, function(col) if (!all(col == col[1])) col else NULL)]
```
...
fwrite(data_out, '../data_after_preprocessing/decoy.csv')
```

# Instructions

Read the report below and use it as a basis for your presentation. You can also download the preprocessed data from `data_after_preprocessing/decoy.csv` and use your favorite software (JASP, jamovi, R, SPSS) to analyze it to get more insight into the study results. The data file has the following columns:
...
plot.pointrange(data_out, aes(x = condition, y = correct, color = decoy_on), wid = 'participant', design = 'w')+labs(y = 'Proportion of correct responses', color = 'Decoy is similar to...', x = 'Condition')+scale_color_manual(values = default_colors)
```
...
plot.pointrange(data_out, aes(x = condition, y = rt, color = decoy_on), wid = 'participant', design = 'w')+labs(y = 'Response time, s', color = 'Decoy is similar to...', x = 'Condition')+scale_color_manual(values = default_colors)
```
