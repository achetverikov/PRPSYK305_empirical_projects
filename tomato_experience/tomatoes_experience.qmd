...
aggr_stats <- tomatoes[,.(.N, circ_sd = circ_sd_360(error, na.rm = T),circ_sd_correct = circ_sd_360(be_c, na.rm = T), n_by_subj=n_by_subj[1], n_nonmissing=sum(!is.na(error)), share_outliers= mean.nn(is_outlier), mean_abs_err= mean.nn(abs_err), bias_to_distr_corr = circ_mean_360(bias_to_distr_corr[target_noise==distr_noise]), bias_rel_to_low_noise = circ_mean_360(bias_rel_to_low_noise, na.rm = T)), keyby = .(subject, task_type = factor(task_type, levels = c('single item','averaging'), labels = c('single_item','averaging')))]#[,mean.nn(share_outliers), by = task_type]


plot.pointrange(tomatoes[is_outlier==F&task_type!='averaging'], aes(x = similarity_binf, y = bias_to_distr_corr, color = target_noise), wid  = 'subject', design = 'w', withinvars  = c('noise_equal') )+facet_grid(~noise_equal)
