...
fwrite(tomatoes, file = 'tomatoes_raw_preprocessed.csv')
```

```{r}
#| eval: false

tomatoes <- fread('tomatoes_raw_preprocessed.csv')
...
tomatoes[!is.na(abs_err) & task_type=='averaging', 
         bias_rel_to_low_noise := sign(angle_diff_360(ifelse(target_noise=='low'&distr_noise=='high', target_color, ifelse(target_noise=='high'&distr_noise=='low', distr_color, NA)), realTargetColor)) * be_c]

aggr_stats <- tomatoes[,.(.N, circ_sd = circ_sd_360(error, na.rm = T),circ_sd_correct = circ_sd_360(be_c, na.rm = T), n_by_subj=n_by_subj[1], n_nonmissing=sum(!is.na(error)), share_outliers= mean_nn(is_outlier), mean_abs_err= mean_nn(abs_err), bias_to_distr_corr = circ_mean_360(bias_to_distr_corr[target_noise==distr_noise]), bias_rel_to_low_noise = circ_mean_360(bias_rel_to_low_noise, na.rm = T)), keyby = .(subject, task_type = factor(task_type, levels = c('single item','averaging'), labels = c('single_item','averaging')))]#[,mean_nn(share_outliers), by = task_type]

tomatoes[,noise_equal:=factor(distr_noise==target_noise, levels = c(F, T), labels = c('Unequal noise', 'Equal noise'))]

plot_pointrange(tomatoes[is_outlier==F&task_type!='averaging'], aes(x = similarity_binf, y = bias_to_distr_corr, color = target_noise), wid  = 'subject', design = 'w', withinvars  = c('noise_equal') )+facet_grid(~noise_equal)
aggr_stats <- dcast(aggr_stats, subject~task_type, value.var = c('circ_sd','circ_sd_correct','n_by_subj','n_nonmissing','share_outliers','mean_abs_err','bias_to_distr_corr','bias_rel_to_low_noise'))
ggplot(aggr_stats, aes(x = `bias_rel_to_low_noise_averaging`, y = bias_to_distr_corr_single_item))+
  geom_point()+
  geom_vline(xintercept = 0, linetype = 2)+geom_hline(yintercept = 0, linetype = 2)+
  geom_abline(slope = 1, intercept = 0, linetype = 2)+
  geom_smooth(method = 'lm')
  # labs(x = 'Circular SD Single Item (degrees)', y = 'Circular SD Averaging (degrees)')
# aggr_stats[,apastats2::apa(cor.test(bias_rel_to_low_noise_averaging,bias_to_distr_corr_single_item))]

tomatoes[,abs_be_c:=abs(be_c)]
tomatoes[,avg_abs_err:=mean_nn(abs(be_c)), by = subject_id]
...
tomatoes_out <- tomatoes[order(subject, trial_number),.(subject, trial_number, target_color, distr_color, left_color, right_color, left_sd,  right_sd, target_noise, distr_noise, noise, similarity_bin, resp_color,  rawError = error, realTargetColor, realErr, errorCorrected = be_c, is_outlier, task_type)]
...
* `task_type` - type of task performed by the participant (factor: "single_item" or "averaging")
...
# Methods
```{r}
#| label: read_aggr_stats
aggr_stats <- fread('tomatoes_aggr_stats.csv')
tomatoes <- fread('tomatoes.csv')
tomatoes[,abs_be_c:=abs(errorCorrected)]
...
abs_error_anova <- ezANOVA(
  ...
  dv = .(abs_be_c),
  wid = .(subject),
  within = .(task_type, similarity_bin),
  detailed = TRUE,
  type = 3
)
...
The repeated-measures ANOVA revealed an effect of the task type `r apa(abs_error_anova, "task_type")` with participants performing better in the averaging task compared to the single-item report. We also found an effect of the similarity bin `r apa(abs_error_anova, "similarity_binf")`, indicating that participants' performance was affected by the similarity between target and distractor colors. The interaction effect was not significant, `r apa(abs_error_anova, "task_type:similarity_bin")`.
...
tomatoes[ task_type=='averaging', 
         bias_rel_to_target := sign(angle_diff_360(target_color, realTargetColor)) * errorCorrected]
tomatoes[ task_type=='averaging', 
         bias_rel_to_target_raw := sign(angle_diff_360(target_color, realTargetColor)) * rawError]

# Create the plot
p <- plot_pointrange(tomatoes[ task_type=='averaging',], 
...
# Run ANOVA on bias
bias_anova <- ezANOVA(
  data = tomatoes[ task_type=='averaging',],
...
The analysis of bias in the averaging task revealed that the effect of similarity bin was not significant, `r apa(bias_anova, "similarity_bin")`, indicating that the similarity between the two colors alone did not systematically affect the direction of participants' errors. However, the effect of noise was significant, `r apa(bias_anova, "noise")`, demonstrating that the variability in the color patches significantly influenced how participants weighted the stimuli when computing the average. Importantly, we found a significant interaction between similarity bin and noise, `r apa(bias_anova, "similarity_bin:noise")`, indicating that the effect of noise on bias was moderated by the similarity between target and distractor colors. This interaction suggests that depending on the noise level, participants weighted the two stimuli differently when computing the average, with higher noise conditions potentially leading to more bias toward the more variable stimuli. 
...
similarity_bins <- unique(tomatoes$similarity_bin)
split_results <- lapply(split(tomatoes[ task_type=='averaging' ], by = 'similarity_bin'), function(dt) {
  ezANOVA(
    data = dt,
    dv = .(bias_rel_to_target),
    wid = .(subject),
...
Separate ANOVAs for each similarity bin showed that the effect of the noise was significant only when items were relatively similar (`r apa(split_results$c20, "noise")` for 20 deg. similarity and `r apa(split_results$c45, "noise")` for 45 deg.) but not when they were very different (`r apa(split_results$c135, "noise")`). With both 20 and 40 similarity, responses were closer to the more variable item, as @fig-bias-analysis shows.
