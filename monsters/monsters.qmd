...
str(data[,.(participant, block_i, expected_length, real_length, lengthDiff, trial_i_within_block, next_wave_in, targetMeanColor, distractorMeanColor, clicked_monster_id, correct, rt)])
...
data[,log_rt:=log(rt)]
plot.pointrange(data[correct==1], aes(x = trial_i_within_blockf, y = rt , color = expected_lengthf ), wid = 'participant', within_subj = F, withinvars=c('trial_i_within_blockf','expected_lengthf'), do_aggregate = T, connecting_line = T)

contrasts(data$trial_i_within_blockf) <- contr.helmert(lengthu(data$trial_i_within_blockf))

rt_fit <- lmerTest::lmer(rt~trial_i_within_blockf+(1+trial_i_within_blockf||participant), data[correct==1])
...
We first confirmed a general 'priming of pop-out' effect. By comparing RT on each trial with the average of subsequent trials, we found significantly longer RT (`r describe.ttest(t_res)`) for the first trial in each sequence ('wave'). However, following the first trial, no significant differences were observed (all _p_ > `r min(t_res[2:8, unlist(p.value)])`). 

```{r}
plot.pointrange(data[correct==1&trial_i_within_block_rel%between%c(-2,2)&lengthDiff!=0&trial_i_within_block_rel!=0], aes(x = trial_i_within_block_rel, y = rel_rt , color = expected_lengthf ), wid = 'participant', within_subj = T, withinvars=c('trial_i_within_block_rel','expected_lengthf'), do_aggregate = F, connecting_line = T, x_as_numeric = T)
data[correct==1&trial_i_within_block_rel==0, ref_rt := rt]
data[,rel_rt:=rt-ref_rt[!is.na(ref_rt)], by = .(participant, block_i)]
data[,prev_ld_positive:=ifelse(prevLengthDiff>0,'longer than expected', ifelse(prevLengthDiff==0, 'as expected', 'shorter than expected'))]
plot.pointrange(data[trial_i_within_block==1&!is.na(prev_ld_positive)], aes(x = prev_ld_positive, y = rt ), wid = 'participant', within_subj = F, do_aggregate = F, connecting_line = T)
```

We then tested how the expectations affect response times. 