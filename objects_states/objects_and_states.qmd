...
data[, state := as.integer(str_extract(final_img, "(?<=s)\\d+"))]
...
num_trials <- length(unique(data$trial_index))
```
...
```{r}
#| label: exemplar-accuracy

# Calculate overall accuracy
mean_accuracy <- data[, mean(correct)]
se_accuracy <- data[, sd(correct)/sqrt(.N)]
...
data[, presented_state := ifelse(correct == 1, state, 3 - state)]

# Group data by trial to analyze pairs of responses
trials_grouped <- data[, .(
  exemplar_1 = .SD[exemplar == 1, exemplar],
  exemplar_2 = .SD[exemplar == 2, exemplar],
  chosen_state_1 = .SD[exemplar == 1, state],
  chosen_state_2 = .SD[exemplar == 2, state],
  presented_state_1 = .SD[exemplar == 1, presented_state],
  presented_state_2 = .SD[exemplar == 2, presented_state],
  correct_1 = .SD[exemplar == 1, correct],
  correct_2 = .SD[exemplar == 2, correct]
), by = .(participant, trial_index, category, tot_correct)]
...
trials_grouped[, response_type := factor(
  ...
  levels = c("Both correct", "One correct", "None correct")
)]
```
...
ggplot(same_state_means, aes(x = factor(same_presented_state, labels = c("Different states", "Same state")), 
                            y = mean_chose_same)) +
  geom_bar(stat = "identity", fill = default_colors[1], width = 0.5) +
  geom_errorbar(aes(ymin = mean_chose_same - se_chose_same, 
...
Participants were able to discriminate between seeing exemplars in the same state versus different states. When exemplars were presented in the same state, participants were more likely to select the same state for both exemplars (`r f.round(same_state_means[same_presented_state == TRUE, mean_chose_same], 2)`), compared to when they were presented in different states (`r f.round(same_state_means[same_presented_state == FALSE, mean_chose_same], 2)`). This difference was significant, `r describe.ttest(t_test_same_diff)`.
...
The key question is whether participants can correctly bind state information to exemplars. Participants were significantly better at remembering the state of each exemplar when the exemplars were shown in the same state (`r f.round(binding_accuracy[same_presented_state == TRUE, mean_accuracy], 2)`) compared to when they were shown in different states (`r f.round(binding_accuracy[same_presented_state == FALSE, mean_accuracy], 2)`), `r describe.ttest(t_test_binding)`.

For exemplars studied in the same state, performance was significantly above chance, `r describe.ttest(t_test_same_vs_chance)`. However, when exemplars were studied in different states, performance was not significantly different from chance, `r describe.ttest(t_test_diff_vs_chance)`.
...
response_distribution <- trials_grouped[, .(
  proportion = .N / sum(.N)
), by = .(same_presented_state, response_type)]

# Plot the distribution of responses
ggplot(response_distribution, 
       aes(x = factor(same_presented_state, labels = c("Different states", "Same state")), 
           y = proportion, 
           fill = response_type)) +
...
To better understand the pattern of responses, we examined the distribution of trials where participants got both exemplars correct, one exemplar correct, or neither correct. In the same state condition, participants were more likely to get both items correct compared to the different state condition. The proportion of trials with one item correct was similar across both conditions, suggesting that the difference in overall accuracy was primarily driven by trials where participants either got both items correct or both incorrect.
## References
