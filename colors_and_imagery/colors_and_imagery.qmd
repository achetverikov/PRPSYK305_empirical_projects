...
data_color[,.(totalN = .N, circ_sd = circ_sd_360(error, na.rm = T),  mean_abs_err= mean.nn(abs_err), n_non_missing_by_subj = n_by_subj[!is.na(error)][1], missed_responses = sum(is.na(error))), keyby = .(participant)]
...
perf_vviq_out <- data_color[,.(mem_perf_n_out = mean.nn(is_outlier)),                                     
                                 by = .(participant, vviq)]
