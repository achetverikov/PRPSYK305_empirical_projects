...
#| label: load_data
data_list <- list()
study_uid <- Sys.getenv('JATOS_COLORS_AND_IMAGERY_STUDY_2026_UID')
...
flist_vviq <- flist[str_extract(flist,'comp-result_(\\d+)', group = 1)%in%comp_res_meta[componentId%in%c('29868','40213')&id!='1067177',id]]
flist_colors <- flist[str_extract(flist,'comp-result_(\\d+)', group = 1)%in%comp_res_meta[componentId%nin%c('29868','40213'),id]]
...
data_color <- drop_empty_cols(data_color)
...
data_color[,.(totalN = .N, circ_sd = circ_sd_360(error, na.rm = T),  mean_abs_err= mean_nn(abs_err), n_non_missing_by_subj = n_by_subj[!is.na(error)][1], missed_responses = sum(is.na(error))), keyby = .(participant)]

```

```{r}
#| label: data_prep_outliers

#outliers 
data_color<-data_color[participant%nin%data_color[,.(.N, circ_sd_360(error, na.rm = T)), by = participant][V2>60|N<250, participant]]
...
data_color[,.(.N, circ_sd = circ_sd_360(error, na.rm = T),circ_sd_correct = circ_sd_360(error, na.rm = T), n_by_subj=n_by_subj[1], n_nonmissing=sum(!is.na(error)), share_outliers= mean_nn(is_outlier), mean_abs_err= mean_nn(abs_err)), keyby = .(participant)]
...
data_color[,avg_abs_err:=mean_nn(abs(be_c)), by = participant]
...
* `is_outlier` - a boolean indicating whether the trial is an outlier to be excluded
...
The VVIQ scores were between `r min(data_vviq$vviq)` and `r max(data_vviq$vviq)` (`r apa_mean_conf(data_vviq$vviq)`).
...
plot_pointrange(data_color[is_outlier==F], aes(x = target_noise, 
                               y = abs_be_c, 
                               color = distr_noise),
               within_subj = TRUE, 
               wid = 'participant', 
...
A repeated-measures ANOVA indicated a significant effect of target noise `r apa(ez_res_abs_err, 'target_noise')` and a target × distractor noise interaction `r apa(ez_res_abs_err, 'target_noise:distr_noise')`. Performance dropped significantly when the targets became more noisy: `r apa(dcast(data_color, participant~target_noise, value.var = 'abs_err', fun = mean)[,t.test(low, high, paired = T)])`. However, the effect of distractor noise was noticeable only when the target had a low noise level `r apa(dcast(data_color[target_noise=='low'], participant~distr_noise, value.var = 'abs_be_c', fun = mean)[,t.test(low, high, paired = T)])` but not when it had a high noise level `r apa(dcast(data_color[target_noise=='high'], participant~distr_noise, value.var = 'abs_be_c', fun = mean)[,t.test(low, high, paired = T)])`.
...
perf_vviq_out <- data_color[,.(mem_perf_n_out = mean_nn(is_outlier)),                                     
                                 by = .(participant, vviq)]
...
Overall, there was no correlation between recall accuracy and VVIQ score (`r perf_vviq[,apa(cor.test(mem_perf_bc, vviq))]`). Furthermore, the drop in performance in the high noise condition compared to the low noise condition also did not correlate with VVIQ scores (`r delta_by_vviq[,apa(cor.test(delta, vviq))]`). Finally, we analyzed the difference in performance between the second item and the first item (where positive values indicate worse performance for the second item), but also did not see any correlation with VVIQ score (`r delta_cue_n_by_vviq[,apa(cor.test(delta, vviq))]`).
