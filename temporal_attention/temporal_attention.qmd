...
costs_benefits <- dcast(data[,.(perf = mean.nn(abs_err)), by = .(participant, cue_type, initial_cue)], participant+initial_cue~cue_type)
costs_benefits[, cost_index := (neutral-invalid)/(valid-invalid)]
costs_benefits[, benefit_index := (valid-neutral)/(valid-invalid)]

costs_benefits <- melt(costs_benefits, id.vars = c('participant', 'initial_cue'), measure.vars = c('cost_index', 'benefit_index')) 

cb_tt <- costs_benefits[,describe.ttest(t.test(value)), by = .(variable, initial_cue)]
plot.pointrange(aes(x = initial_cue, y = value, color = variable), 
                  within_subj = TRUE,
                  design = 'w',
                  wid = "participant",
                  ...
                  pointsize = 3,
                  linesize = 1) + labs(
                    x = "Initial Cue Position", color = "Index Type",
...
Finally, we repeated the cost-benefit analysis introduced by Denison et al. (2017). As @fig-benefit shows, the results were relatively noisy in case when pre-cue was 1 or 2, and only for the last target we found reliable costs (`r cb_tt[variable=='cost_index'&initial_cue=='3', V1]`).
