...
* no main effect of initial cue position: `r apa(position_anova, 'initial_cue')`
...
Finally, we repeated the cost-benefit analysis introduced by Denison et al. (2017). As @fig-benefit shows, for target 1 and 3, there were some costs (`r cb_tt[variable=='cost_index'&post_cue%in%c(1,3), paste_and(V1)]`) along with some benefits that did not reach significance (`r cb_tt[variable=='benefit_index'&post_cue%in%c(1,3), paste_and(V1)]`). No signficant effects were found for target 2 in this type of analysis.
