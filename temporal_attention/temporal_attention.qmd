...
export_cols <- c(
    'participant', 'block', 'trial_number', 't1_ori', 't2_ori', 't3_ori',
    'target_position', 'cue_type', 'real_target_position', 'start_ori',
    'resp_ori', 'error', 'abs_err', 'target_ori', 'diff_to_next', 'diff_to_prev'
  )
fwrite(data[, ..export_cols], file = 'temporal_attention.csv')
```
...
### Participants
`r stringr::str_to_sentence(xfun::numbers_to_words(data[,lengthu(participant)]))` participants took part in this study. Participation required a web browser with a minimum screen resolution of 1000 × 600 pixels. Demographic information was not collected.
...
The effect of a pre-cue type on accuracy was analyzed using a within-subject ANOVA with absolute error in orientation judgment as the dependent variable (@fig-cue-type-effect). The results show a clear effect of cue type: `r apa(cue_type_anova, 'cue_type')`. On average, participants average absolute error was `r data_descr[,apa_mean_conf(acc), by = .(cue_type)][,paste_and(sprintf('%s with %s cues', V1, cue_type))]` (here and later the confidence intervals are reported in square brackets after the means).
...
pre_post_by_subj <- pre_post[,mean_nn(abs_err), by = .(participant, post_cue, initial_cue)]
ttest_descr <- apa(dcast(pre_post_by_subj, participant~initial_cue, fun=mean)[,t.test(`1`,`3`, paired = T)])
...
* a significant main effect of initial cue position: `r apa(position_anova, 'initial_cue')`
* a significant main effect of post-cue position: `r apa(position_anova, 'post_cue')`
* a significant interaction effect: `r apa(position_anova, 'initial_cue:post_cue')`
...
costs_benefits <- dcast(data[,.(perf = mean_nn(abs_err)), by = .(participant, cue_type, post_cue)], participant+post_cue~cue_type)
...
cb_tt <- costs_benefits[,.(V1 = apa(t.test(value))), by = .(variable, post_cue)]
plot_pointrange(costs_benefits, aes(x = post_cue, y = value, color = index_type), 
                  within_subj = TRUE,
                  design = 'w',
                  wid = "participant",
