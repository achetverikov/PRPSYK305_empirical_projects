...
```{r results='asis' }
print(data[,.N, by = participant])
```

```{r}
#| fig-cap: Performance a function of cue validity
#| label: fig-perf

plot.pointrange(data, aes(x = cue_type, y = abs_err))
```

```{r}
data[,real_target_positionf:=factor(real_target_position, levels = 1:3)]
ggplot(data, aes(x = diff_to_next, y = error, color = real_target_positionf))+geom_smooth()+gg_crosshair()+scale_color_discrete(drop=F)+
ggplot(data, aes(x = diff_to_prev, y = error, color = real_target_positionf))+geom_smooth()+gg_crosshair()+scale_color_discrete(drop=F)+
